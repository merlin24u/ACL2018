	...
	public Monster createMonster(String type, Map map) {
		...
		switch(type) {
			case "warrior":
				...
				groundCollisionHandler = GroundCollisionHandlerFactory.getInstance().getGroundCollisionHandler(map, collisions);
		...
		return new Monster(damageEffectFactory, movableArtificialIntelligence, 
				currentHP, maximumHP, defensePoints, 
				groundCollisionHandler,
				movingSpeedXMax, movingSpeedYMax, movingTick, new Point(), texture);
	}
