...
public class GroundCollisionHandler {
	...
	public void handleMove(Movable movable) {
		...
		int nextPositionY = (int) (movable.getPosition().getY() + movable
				.getCurrentSpeedY());
		if (nextPositionX >= 0 && nextPositionX < map.getWidth()
				&& nextPositionY >= 0 && nextPositionY < map.getHeigh()) {
			if (map.getValue(nextPositionX, nextPositionY) != ECollisionType.WALL
					.getValue()) {
